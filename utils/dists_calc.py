...
def get_dists_model():
    try:
        ...
        os.system('pip install DISTS_pytorch')
        from DISTS_pytorch import DISTS
    return DISTS()

def calc_dists_metric(model, device, frames_cnt, dist_frames_path = None, gt_frames_path = None):
    score = 0.0
    full_time = 0.0
    ...
    for i in tqdm(range(frames_cnt)):
        ...
        try:
            ...
            score += model.score(process_image(iio.imread(small_dist_image), device)[None, ...], process_image(iio.imread(small_gt_image), device)[None, ...]).item()
            start_time = time.time() - start_time
            full_time += start_time
        except:
            start_time = time.time()
            score += model.score(process_image(iio.imread(dist_image), device)[None, ...], process_image(iio.imread(gt_image), device)[None, ...]).item()
            start_time = time.time() - start_time
            full_time += start_time
